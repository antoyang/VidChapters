...
class PDVC(nn.Module):
    ...
    def __init__(self, base_encoder, transformer, captioner, num_classes, num_queries, num_feature_levels,
        ...
        if opt.caption_decoder_type == 'gpt2':
            self.query2cap = nn.Linear(transformer.d_model, captioner.config.n_embd)
    ...
    def caption_prediction(self, cap_head, dt, hs, reference, others, captioner_type, indices=None):
        ...
        elif captioner_type in ['light', 'gpt2']:
            ...
            if self.training:
                ...
                if self.opt.caption_cost_type != 'rl':
                    if self.opt.caption_decoder_type == 'gpt2':
                        inputs_embeds = cap_head.transformer.wte(seq)
                        visual_prefix = self.query2cap(clip_ext)
                        inputs_embeds = torch.cat([visual_prefix, inputs_embeds], 1)
                        labels = torch.cat([torch.ones(seq.shape[0], dtype=torch.long).to(seq.device)[:, None] * -100,
                                            seq.masked_fill(seq == 0, -100)], dim=1)
                        output = cap_head(inputs_embeds=inputs_embeds, labels=labels)
                        cap_cost = cap_loss = output.loss
                        cap_prob = output.logits
                    else:
                ...
                with torch.no_grad():
                    ...
                    if self.opt.caption_decoder_type == 'gpt2':
                        ...
                        cap_cost = cap_loss = output.loss
                        cap_prob = output.logits
                        import pdb;pdb.set_trace()  # useless path right?
                    else:
        ...
        if self.opt.caption_cost_type == 'loss' and self.opt.caption_decoder_type != 'gpt2':
    ...
    def caption_prediction_eval(self, cap_head, dt, hs, reference, others, decoder_type, indices=None):
        ...
        if decoder_type in ['none']:
            ...
            with torch.no_grad():
                clip = hs_r.unsqueeze(1)
                inputs_embeds = self.query2cap(clip)
                output = cap_head.generate(inputs_embeds=inputs_embeds,
                                           max_length=self.opt.max_caption_length,
                ...
                cap_prob_eval = output.sequences_scores * (seq > 0).sum(-1)
                seq = seq.reshape(-1, N_q, seq.shape[-1])  # B N_q 30
                cap_prob_eval = cap_prob_eval.reshape(-1, N_q)  # B N_q
                cap_probs['cap_prob_eval'] = cap_prob_eval
    ...
    def forward(self, outputs, target_sizes, loader):
        ...
        if len(seq):
            if self.opt.caption_decoder_type != 'gpt2':
                ...
                cap_scores = cap_prob.cpu().numpy().astype('float')
            ...
            if self.opt.caption_decoder_type != 'gpt2':
                ...
                caps = [loader.dataset.translator.batch_decode(s, skip_special_tokens=True) for s in seq]
...
def build(args):
    ...
    criterion = SetCriterion(args.num_classes, matcher, weight_dict, losses, focal_alpha=args.focal_alpha,
                             focal_gamma=args.focal_gamma, opt=args, self_iou=args.self_iou)
    criterion.to(device)
