...
def eval_metrics(dvc_filename, gt_filenames, para_gt_filenames, alpha=0.3, ranking_key='proposal_score', rerank=False, dvc_eval_version='2021'):
    ...
    for i, x in enumerate([0.3, 0.5, 0.7, 0.9]):
        for y in ["Recall", "Precision", "F1"]:
            loc_score[y + "@" + str(x)] = dvc_score[y][i]
    for y in ["Recall", "Precision", "F1"]:
        loc_score[y] = np.array([dvc_score[y][i] for i in range(4)]).mean()
    for i, x in enumerate([1, 3, 5, 10, 30, 60]):
        for y in ["Recall", "Precision", "F1"]:
            loc_score[y + "@" + str(x) + "s"] = dvc_score[y][4 + i]
    dvc_score = {key: np.array(value).mean() for key, value in dvc_score.items() if
                      key not in ["Recall", "Precision", "F1"]}
    dvc_score.update(loc_score)
    # dvc_score = {k: sum(v) / len(v) for k, v in dvc_score.items()}
    dvc_score.update(eval_soda(dvc_filename, ref_list=gt_filenames))
    if para_gt_filenames:
...
def evaluate(model, criterion, postprocessors, loader, dvc_json_path, logger=None, score_threshold=0,
             alpha=0.3, dvc_eval_version='2021', device='cuda', debug=False, skip_lang_eval=False):
    out_json = {'results': {},
                'version': "VERSION 1.0",
