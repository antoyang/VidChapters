...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.3, 0.95, 14), dists=[1, 3, 5, 10]):
    ...
    for dist in dists:
        iou_thd2recall_at_one[str(dist) + "s"] = float(f"{np.mean(np.abs(pred_windows[:, 0] - gt_windows[:, 0]) <= dist) * 100:.2f}")
...
def eval_moment_retrieval(submission, ground_truth, verbose=True, dataset="hl"):
    if dataset=="charades":
        ...
        length_ranges = [[0, 30], [30, 60], [60, 800], [0, 800], ]
        range_names = ["short", "middle", "long", "full"]
    else:
        # length_ranges = [[0, 10], [10, 30], [30, 150], [0, 150], ]
        length_ranges = [[0, 60], [60, 180], [180, 3600], [0, 3600], ]
        range_names = ["short", "middle", "long", "full"]
...
def eval_submission(submission, ground_truth, verbose=True, match_number=True, dataset="hl"):
    ...
    if "pred_relevant_windows" in submission[0]:
        ...
        moment_ret_scores_brief = {
            ...
            "MR-long-mAP": moment_ret_scores["long"]["MR-mAP"]["average"] if "long" in moment_ret_scores else 0.,
            ...
            "MR-full-R1@1s": moment_ret_scores["full"]["MR-R1"]["1s"],
            "MR-full-R1@3s": moment_ret_scores["full"]["MR-R1"]["3s"],
            "MR-full-R1@5s": moment_ret_scores["full"]["MR-R1"]["5s"],
            "MR-full-R1@10s": moment_ret_scores["full"]["MR-R1"]["10s"],
        }
        eval_metrics_brief.update(
            sorted([(k, v) for k, v in moment_ret_scores_brief.items()], key=lambda x: x[0]))

    if "pred_saliency_scores" in submission[0] and dataset=="hl":
