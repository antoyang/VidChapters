...
@torch.no_grad()
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None, tokenizer=None):
    model.eval()
    if criterion:
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        query_meta = batch[0]
        model_inputs, targets = prepare_batch_inputs(batch[1], opt.device, non_blocking=opt.pin_memory, tokenizer=tokenizer, speech_max_l=opt.max_speech_l)
    ...
    # )
    post_processor = PostProcessorDETR(
            clip_length=opt.clip_length, min_ts_val=0, max_ts_val=opt.max_v_l if not opt.downsample else opt.max_v_l * opt.clip_length,
            min_w_l=opt.clip_length, max_w_l=opt.max_v_l if not opt.downsample else opt.max_v_l * opt.clip_length, move_window_method="left",
            process_func_names=("clip_ts", "round_multiple")
        )
...
def get_eval_res(model, eval_loader, opt, epoch_i, criterion, tb_writer, tokenizer):
    """compute and save query and video proposal embeddings"""
    eval_res, eval_loss_meters = compute_mr_results(model, eval_loader, opt, epoch_i, criterion, tb_writer, tokenizer)  # list(dict)
    return eval_res, eval_loss_meters


def eval_epoch(model, eval_dataset, opt, save_submission_filename, epoch_i=None, criterion=None, tb_writer=None, tokenizer=None):
    ...
    submission, eval_loss_meters = get_eval_res(model, eval_loader, opt, epoch_i, criterion, tb_writer, tokenizer)
...
def start_inference():
    ...
    with torch.no_grad():
        metrics_no_nms, metrics_nms, eval_loss_meters, latest_file_paths = \
            eval_epoch(model, eval_dataset, opt, save_submission_filename, criterion=criterion, tokenizer=tokenizer)
