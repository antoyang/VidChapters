...
class MomentDETR(nn.Module):
    ...
    def __init__(self, clip_model, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
                 num_queries, input_dropout, aux_loss=False,
                 contrastive_align_loss=False, contrastive_hdim=64,
                 max_v_l=75, span_loss_type="l1", use_txt_pos=False, n_input_proj=2, use_speech=False, freeze_speech=False, use_video=True):
        ...
        if use_speech:
            ...
            self.bert_proj = nn.Linear(768, transformer.d_model)
            if freeze_speech:
    ...
    def encode_text(self, text):
        ...
        x = torch.matmul(x.unsqueeze(1), self.clip_model.text_projection.unsqueeze(-1)).squeeze(-1)

        return x.permute(0, 2, 1)

    def forward(self, tokens, src_vid, src_vid_mask, speech_tokens=None):
        ...
        src_txt_mask = tokens != 0
        src_txt = self.encode_text(tokens).float()
        src_txt = self.input_txt_proj(src_txt)
        if self.use_speech:
            src_speech = self.bert(speech_tokens).last_hidden_state
            src_speech = self.bert_proj(src_speech)
            if self.use_video:
                src = torch.cat([src_vid, src_txt, src_speech], dim=1)  # (bsz, L_vid+L_txt, d)
                mask = torch.cat([src_vid_mask, src_txt_mask, speech_tokens != 0], dim=1).bool()  # (bsz, L_vid+L_txt)
            else:
                src = torch.cat([src_txt, src_speech], dim=1)  # (bsz, L_vid+L_txt, d)
                mask = torch.cat([src_txt_mask, speech_tokens != 0], dim=1).bool()  # (bsz, L_vid+L_txt)
        else:
        ...
        if self.use_speech:
            pos_speech = torch.zeros_like(src_speech)
            if self.use_video:
                pos = torch.cat([pos_vid, pos_txt, pos_speech], dim=1)
            else:
                pos = torch.cat([pos_txt, pos_speech], dim=1)
        else:
...
def build_model(args):
    ...
    clip_model, _ = clip.load("ViT-L/14", download_root="TOFILL", device=device)
    ...
    model = MomentDETR(
        ...
        freeze_speech=args.freeze_speech,
